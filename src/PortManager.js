...
class PortManager extends EventEmitter {
  ...
  /**
   * ready event
   * The port is initialized, and knows the deviceID of the deviceID it is connected to
   *
   * @event PortManager#ready
   * @type {object}
   * @property {string} deviceID - The deviceID of the ready device
   */
   ...
   * @type {object}
   * @property {string} deviceID - The deviceID of the disconnected device
   */

  /**
   * idchange event
   * Upon reconnection, the deviceID of the device connected to the port has changed
   *
   * @event PortManager#idchange
   * @type {object}
   * @property {string} deviceID -  The new device deviceID
   */

  /**
   * reinitialize event
   * Upon reconnection, the deviceID of the device connected to the port is still the same
   *
  ...
  addRequest(cmd, options) {
    options = options || {};
    if (!this.ready && cmd !== this.options.getIdCommand)
      return Promise.reject(new Error("Device is not ready"));
    if (this.queueLength > this.options.maxQLength) {
      debug("max Queue length reached for device :", this.deviceId);
      return Promise.reject(
        new Error(
          "Maximum Queue size exceeded, wait for commands to be processed"
        )
      );
    }
    this.queueLength++;
    debug("adding request to serialQueue for device :", this.deviceId);
    debug("number of requests in Queue :", this.queueLength);
    this.lastRequest = this.lastRequest.then(
      this._appendRequest(cmd, options.timeout),
      this._appendRequest(cmd, options.timeout)
    );
  ...
  _serialPortInit() {
    ...
    this.addRequest(this.options.getIdCommand)
      .then((buffer) => {
        ...
        if (!deviceId) {
          throw new Error("Device deviceID parsing returned empty result");
        } else if (this.deviceId && this.deviceId !== deviceId) {
          this.deviceId = deviceId;
          debug(`Device Id changed to: ${deviceId}`);
          this.emit("idchange", { deviceID: this.deviceId });
          ...
          this.deviceId = deviceId;
          this._updateStatus(2);
          this.emit("reinitialized", { deviceID: this.deviceId });
  ...
  _updateStatus(code, message) {
    ...
    switch (this.statusCode) {
      ...
      case 1:
        ...
        this.status = "getting device deviceID";
        break;
      case 2:
      ...
      this.currentRequest = new Promise((resolve, reject) => {
        ...
        if (this.deviceId !== null && cmd !== this.options.getIdCommand) {
          if (callId !== this.deviceId) {
            _reject(new Error("invalid deviceID"));
            return;
          }
        }
          ...
          if (bufferSize < that.buffer.length || force) {
            ...
            if (that.options.checkResponse) {
              if (!that.options.checkResponse(that.buffer)) {
                debug(
                  "The device response to the command did not pass validation",
                  JSON.stringify(that.buffer)
                );
                _reject(
                  new Error(
                    "The device response to the command did not pass validation"
  ...
  _handleWriteError(err) {
    ...
    this.port.close(() => {
      debug(
        "Connection to serial port failed, closing connection and retrying in 2 seconds" +
          err
      );
  ...
  _reconnectionAttempt() {
    debug("reconnection attempt: " + this.path);
    this._hasPort().then(
      () => {
        this._updateOptions();
        this.port = new SerialPort(this.path, this.options);
        this.port.on("open", () => {
          debug("opened port:", this.path);
        ...
        this.port.on("error", (err) => {
          this._updateStatus(-1);
          debug(`serialport error on ${this.path}: ${err.message}`);
          this.emit("error", err);
          this._tryLater();
        });
        ...
        this.port.on("disconnect", (err) => {
          ...
          debug(`serialport disconnect on port ${this.path}: ${err.message}`);
          this.emit("disconnect", { deviceID: this.deviceId });
        });

        this.port.on("close", (err) => {
          this._updateStatus(4);
          debug(`serialport close on port ${this.path}`);
          this.emit("close", err);
          this._reconnectionAttempt();
        });
  ...
  async _hasPort() {
    ...
    let ports = await SerialPort.list().catch((err) => debug(err));
    ...
    new Error(`Port ${this.path} not found`);
  }
  ...
  _tryLater() {
    debug(
      ...
      this.path,
      ". Please check if your device is connected or your device configuration. We will retry connecting every 2 seconds"
    );
    setTimeout(() => {
      this._reconnectionAttempt();
    }, 2000);
